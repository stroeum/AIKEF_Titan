...
//!end comments
		...
		if(is_box_boundary[_OBS_] && !out_of_Box)
		{
			...
			if(use_Star==true)
			dist_to_orig_obs2 = sqrt( (x[0]-Position_Star[0])*(x[0]-Position_Star[0])
						+ (x[1]-Position_Star[1])*(x[1]-Position_Star[1]) 
						+ (x[2]-Position_Star[2])*(x[2]-Position_Star[2])
						);
			
		
			if(r < (R_Obstacle*R_Obstacle) || (dist_to_orig_obs2 > 0 && dist_to_orig_obs2 < R_Star) )
			{
...
//!-------------------------------------------------------------//
void CBlock::delete_too_lightweight_particles(INT64 *num_too_lightweight_particles, INT32 level)
{
	 ...
	 for(ind[0] = 1; ind[0] < BlkNds_X-1; ind[0]++)
	  for(ind[1] = 1; ind[1] < BlkNds_Y-1; ind[1]++)
	   ...
	   {       
		...
		//! do not consider cells within obstacle
		//! it is approx    num_MPiC[species][i_j_k] * weight = density
		//! thus just go into cells where min_weight = 0.1 average_weight, i.e.
		//! 		num_MPiC[species][i_j_k] * 10*min_weight < density
		if(!Flag[i_j_k] && rho[i_j_k] > 10*num_MPiC[species][i_j_k]*min_particle_weight)
		{
			for(INT32 part_index=0; part_index<num_MPiC[species][i_j_k]; part_index++)
			{		
				active_particle = pArray[species][i_j_k] +part_index;
						
				if(active_particle->weight<min_particle_weight)
				{	
					delete_particle(species, i_j_k, part_index);
										
					num_too_lightweight_particles[level]++;
				}			
			}
