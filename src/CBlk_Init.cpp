...
void CBlock::init_blk_boundaries(void)
{
	...
	//! Marc as Boundary Block if necessary
...
void CBlock::alloc_process_specific_Memory(void)
{
	...
	//! Curl Free Magnetic Backround Field  (Array 16-18)
	Field_Type[id_Bcfbg_HIMesh] = Field_Type[id_Bcfbg] +3*num_nodes_in_block; 

	//! Electric Field (Array 19-21)
	...
	//!  (Arrays 29 + 14*num_Charged_Species)
	...
	for(int species=0; species<num_Charged_Species; species++) 
        ...
        //! PI for all species (Array 29 + num_Charged_Species + 1 -- +)
	...
	//! id_Refine_Rating  (Array 29 + 14*num_Charged_Species + 16)
	Field_Type[id_Refine_Rating] = Field_Type[id_rotB] +3*num_nodes_in_block;
	
	//! id_scratch  (Array 29 + 14*num_Charged_Species + 17-20)
...
#else
	//! id_scratch  (Array 29 + 14*num_Charged_Species + 21 + 6*num_Neutral_Species)
	...
	//!-------------------------------------------------------------
      	//! extern fields  (Array 29 + 14*num_Charged_Species + 21 + 6*num_Neutral_Species + 4*num_Dust_Species + 4*num_ion_prod_fields + 4*num_externRhoVelocityFields)
	//!--------------------------------------------------------------
	...
	//! average fields
      	//! extern fields  (Array 29 + 14*num_Charged_Species + 21 + 6*num_Neutral_Species + 4*num_Dust_Species + 4*num_ion_prod_fields + 4*num_externRhoVelocityFields + num_scalar_average_fields)
...
//!-------------------------------------------------------------//
//! calc_en_inelastic_collision_rate
//! See Diss CK
//! Uses only the first neutral species, must be water!
//!-------------------------------------------------------------//
void CBlock::calc_en_inelastic_collision_rate(INT32 dest, INT32 electronTemp)
{
    D_REAL* Rate = Field_Type[dest];
    D_REAL* T_e = Field_Type[electronTemp];
    
    D_REAL* n_n = Field_Type[id_numberdensity_neutralSpecies1+en_water_neutral_species];
    D_REAL* p_n = Field_Type[id_pressure_neutralSpecies1+en_water_neutral_species];
    
    //! T_n = p_n^ast * p_0 / (n^ast * n_0 * k_b) = p_n^ast / (n^ast * calcBeta)
    
    for (INT32 node=0; node < num_nodes_in_block*COMPs_FType[dest]; node++)
    {
        //! Rotational Excitations
        D_REAL T_n = p_n[node]/(n_n[node]*calcBeta);
        D_REAL fermi_profile = 2.e-9*(1-2/(exp((T_e[node]-T_n)/en_in_coll_delta_t)));
        
        //! Electronic Excitations in case T_e>10^4
        if(T_e[node]>1.e3)
        {
            D_REAL electronic_loss = 0;
            
            if(T_e[node]>1.e6)
            {
                electronic_loss = 3.30422e-6;
            }
            else
            {
                D_REAL logT_e = log(T_e[node]);
                electronic_loss = exp(-188.4701+33.2547*logT_e-2.0729*logT_e*logT_e+0.0425*logT_e*logT_e*logT_e);
            }
            Rate[node] = fermi_profile+electronic_loss/(1 + 2.e-4*T_e[node]);
        }
        else
        {
            //! In case Electronic Excitations can be neglected
            Rate[node] = fermi_profile;
        }
        //! Until now calculation in cm^3 eV/s
        //! 1eV = 1.602e-19 J
        //! 1 cm^3 = 10^-6 m^3
        Rate[node]*= (1 + 2.e-4*T_e[node])*1.602e-19*1e-6*SI_t0*SI_n0*SI_n0*2*mu_0/(SI_B0*SI_B0);
...
void CBlock::calc_CFBG_Vector_with_Parameters(D_REAL* Vector, const D_REAL* moment, const D_REAL* angle, const D_REAL* offset, D_REAL* position)
{
//		if(mpi_myRank==0) printf("m_Star = [%e %e %e]\n",moment[0],moment[1],moment[2]);
//		exit(1);

		//! convert degree to rad
		D_REAL x_angle = M_PI / 180. *angle[0];
		D_REAL z_angle = M_PI / 180. *angle[1];
		...
		position[0] = pos_RotZ[0] - offset[0];
		position[1] = pos_RotZ[1] - offset[1];
		position[2] = pos_RotZ[2] - offset[2];

		//! --get-r--------------
		D_REAL r = vec_len(position);

		D_REAL M_r = (   position[0] * moment[0]
				+position[1] * moment[1]
				+position[2] * moment[2]);
		...
		if (r!=0) {
		  Vector[0] = (3.*(M_r)*position[0])/r_5 -moment[0]/r_3;
		  Vector[1] = (3.*(M_r)*position[1])/r_5 -moment[1]/r_3;
		  Vector[2] = (3.*(M_r)*position[2])/r_5 -moment[2]/r_3;
		} else {
		  Vector[0] = 0.;
		  Vector[1] = 0.;
		  Vector[2] = 0.;
...
void CBlock::set_Fields(void)
{
	...
	if(vec_len(Magnetic_Moment)!=0. || vec_len(Magnetic_Moment_Star)!=0.)
	for(INT32 u=start[0]; u< BlkNds_X-end[0];u++)
	  for(INT32 v=start[1]; v< BlkNds_Y-end[1];v++)
		...
		calc_CFBG_Vector_with_Parameters(CFBG_Vector, Magnetic_Moment, Magnetic_Moment_Angle, Magnetic_Moment_Offset, r_vec);
		...
		if(use_Star==true) {
			for (INT32 m=0; m<3; m++) r_vec[m] -= Position_Star[m];
			calc_CFBG_Vector_with_Parameters(CFBG_Vector, Magnetic_Moment_Star, Magnetic_Moment_Angle_Star, Magnetic_Moment_Offset_Star, r_vec);
			B1_cfbg[u_v_w] += CFBG_Vector[0];
			B2_cfbg[u_v_w] += CFBG_Vector[1];
			B3_cfbg[u_v_w] += CFBG_Vector[2];
		}
		...
		if(mesh_type==STAGGERED)
		{
			...
			calc_CFBG_Vector_with_Parameters(CFBG_Vector, Magnetic_Moment, Magnetic_Moment_Angle, Magnetic_Moment_Offset, r_vec_HIMesh);
			...
			if(use_Star==true) {
				for (INT32 m=0; m<3; m++) r_vec_HIMesh[m] -= Position_Star[m];
				calc_CFBG_Vector_with_Parameters(CFBG_Vector, Magnetic_Moment_Star, Magnetic_Moment_Angle_Star, Magnetic_Moment_Offset_Star, r_vec_HIMesh);
				B1_cfbg[u_v_w] += CFBG_Vector[0];
				B2_cfbg[u_v_w] += CFBG_Vector[1];
				B3_cfbg[u_v_w] += CFBG_Vector[2];
			}	
...
// 				}
		...
		if(use_Star==true)
		{	
			D_REAL x_shift = Position_Star[0];
			D_REAL y_shift = Position_Star[1];
			D_REAL z_shift = Position_Star[2];
