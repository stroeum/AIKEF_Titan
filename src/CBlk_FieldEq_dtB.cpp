		...
		PE_new[i_j_k] +=  dt_step *
		  (
	       ...
	       if(en_use_inelastic_collisions)
                {
                    //! Use only for the Neutral Species with number "en_water_neutral_species", since it is only true for water
                    if(species==en_water_neutral_species)
                    {
                        D_REAL* en_in_coll = Field_Type[id_in_elastic_en_coll_freq];
                        if( rRHO[i_j_k] >= 10.*PART_REAL_PRECISION )
                        {
                            PE_new[i_j_k] +=  dt_step *
                            (
                                //!------ 2/3 n_e* n_n * en_in_coll ---
                                -2. * neutral_n[i_j_k]*en_in_coll[i_j_k]/(rRHO[i_j_k]*3.)
                            );
                        }
                    }
                }
