...
if(TL_PROTOCOL_ANY_PARTICLE && TL%TL_PROTOCOL_ANY_PARTICLE==0)
{
	...
	memset(num_in_obst, 0,num_Particle_Species *sizeof(INT64));
	memset(num_out_of_Box, 0,num_Particle_Species *sizeof(INT64));
...
// 	wait_time = (double(finish)-double(start_wait))/CLOCKS_PER_SEC;
	...
	INT64 local_info_values[8 +2*num_Particle_Species +(MAX_LEVEL+1)];
	...
	for(INT32 species=0; species<num_Particle_Species; species++)
	{
		...
		info_names[8 +num_Particle_Species +species] << "    ->deleted (out Box) " 
					<<Field_Name[id_rhoSpecies1 +species]<<": ";
		local_info_values[8 +num_Particle_Species +species] = num_out_of_Box[species];
	}
	...
	for(int z=0; z<= MAX_LEVEL; z++)
	{
		...
		info_names[8 +2*num_Particle_Species +z] << "     -> L[" << z << "]:  ";
		local_info_values[8 +2*num_Particle_Species +z] = num_total_particles_in_L[z];
	}

	show_information(local_info_values,
			 info_names,
			 8 +2*num_Particle_Species +(MAX_LEVEL+1),
			 BUILD_SUM);
...
void CHybrid::advanceB_LF_globalDT(void)
{
	...
	apply_new_BField_Boundaries();
       ...
       //! Calculate In Elastic Collision
        if(en_use_inelastic_collisions)
        {
...
//!------------------------------------------------------------------------
void CHybrid::calc_en_inelastic_collision_rate()
{
    
    
    //! First Update Electron Temperature
    calc_electron_temperature(id_ElectronTemperature);
    
    //! Calc Collision Rate
    INT32 field = id_in_elastic_en_coll_freq;
    log_file << "  calc " << Field_Name[field] << "...       ";
    set_zero_field(field);
    for(INT32 level=0; level<= MAX_LEVEL; level++)
    {
        ...
        while(temp_Block)
        {
            ...
            if(mpi_myRank == temp_Block->responsible_mpi_process)
                temp_Block->calc_en_inelastic_collision_rate(field,id_ElectronTemperature);
...
//!------------------------------------------------------------
//! delete_too_lightweight_particles:
//!
//! - Check how many particles have weight smaller than min_weight
//! - delete these particle
//!------------------------------------------------------------
void CHybrid::delete_too_lightweight_particles(void)
{
  
	log_file << " Delete macroparticles which give only negligible contribution to density: " <<endl;

	clock_t start,finish;
	double time;
	start = clock();

	//! array for counting deleted particles
	INT64* num_too_lightweight_particles = new INT64[MAX_LEVEL + 1]; 
	memset(num_too_lightweight_particles,0,(MAX_LEVEL + 1)*sizeof(INT64));
	...
	for(INT32 level=0; level<=MAX_LEVEL; level++)
	{
		...
		while(temp_Block)
		{
			...
			if(mpi_myRank == temp_Block->responsible_mpi_process)
			temp_Block->delete_too_lightweight_particles(num_too_lightweight_particles, level);
	...
	//! Provide Information about how many particles do have too low weight
	INT64 local_info_values[MAX_LEVEL+1];
	stringstream info_names[MAX_LEVEL+1];	
	
// 	INT64 num_too_lightweight_particles_all_levels = 0;
	
	for(INT32 level=0; level<=MAX_LEVEL; level++)
	{		
		local_info_values[level] = num_too_lightweight_particles[level];
		info_names[level] << "   -> Num particles with weight < min_particle_weight in level "<<level<<" : ";
		
// 		num_too_lightweight_particles_all_levels += num_too_lightweight_particles[level];
	}	
	...
	delete[] num_too_lightweight_particles;

	//! measure time
	finish = clock();
	time = (double(finish)-double(start))/CLOCKS_PER_SEC;
	log_file << " time for deleting these particles: " << time << "s." << endl << endl;
