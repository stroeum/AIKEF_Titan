...
#define chunk_factor 2  //factor used for the chunking of HDF5 files
...
ifstream Particle_inFile;
 ...
 bool FileExists(char* filename) //Function to check whether a file already exists
{
...
void CHybrid::trajectory_output(void)
{
	...
	for(INT32 flyby=0; flyby<num_trajectories; flyby++)
	  ...
	  {	
		trace_field_flyby[flyby][field]=false;
...
//		silo_3DWrite_Field(id_scratch_scalar);
	...
	for(int species=0; species<num_Particle_Species; species++)
	{
		...
		//! calc gyroradius ions
		calc_local_gyro_radius(id_gyro_Species1 + species,species,id_BTotal,id_UI_Species1 +species);
		silo_3DWrite_Field(id_gyro_Species1 +species);
		//! calc gyroradius electrons
		calc_local_electron_gyro_radius(id_gyro_el_Species1 + species,species,id_BTotal,id_UI_Species1 +species);
		silo_3DWrite_Field(id_gyro_el_Species1 +species);
...
// 		calc_div(id_UI_plus, id_PISpecies1 +0);
// 		FULL_GN_UPDATE(id_PISpecies1 +0);
// 		silo_3DWrite_Field(id_PISpecies1 +0);
	...
	for(INT32 species=0; species<num_Neutral_Species; ++species)
	{
		...
		//! write number density
		silo_3DWrite_Field(id_numberdensity_neutralSpecies1     + species);
		
		//! write bulk velocity
		silo_3DWrite_Field(id_velocity_neutralSpecies1          + species);
		
		//! write pressure
		silo_3DWrite_Field(id_pressure_neutralSpecies1          + species);
		
		//! write new electron beta
		silo_3DWrite_Field(id_new_electron_beta_neutralSpecies1 + species);
	...
	for(INT32 species=0; species<num_ion_prod_fields; ++species)
	{
		...
		//! write number density
		silo_3DWrite_Field(id_density_ionProdSpecies1     + species);

		//! write bulk velocity
		silo_3DWrite_Field(id_velocity_ionProdSpecies1          + species);
	...
	for(INT32 species=0; species<num_Dust_Species; ++species)
	{
		...
		//! write number density
		silo_3DWrite_Field(id_density_dustSpecies1     + species);

		//! write bulk velocity
		silo_3DWrite_Field(id_velocity_dustSpecies1    + species);
	...
	if(!TestParticle_Simulation)
	for(INT32 average_field=0; average_field<num_average_fields; average_field++)
	silo_3DWrite_Field(id_average_Field1 +average_field);
...
//!--------------------------------------------------------
//!- save state- function to switch between old and hdf format
//!--------------------------------------------------------
void CHybrid::save_state(void)
{
 if (write_hdf_state) hdf_save_state(); else old_save_state();
}
...
//!--------------------------------------------------------
void CHybrid::old_save_state(void)
{
...
// 	}
	...
	log_file << " Saving state to binary file..." << endl;
	synchronize_allProcesses_MPI();	
...
//!--------------------------------------------------------
//!- restore_state - function to switch between old and hdf format
//!--------------------------------------------------------
bool CHybrid::restore_state(void)
{
 if (read_hdf_state) {
	//! restore Blks before children are created
    	if(!hdf_restore_state()) return false; else return true;
 } else {
   if(!old_restore_state()) return false; else return true;
 }
}

//!--------------------------------------------------------
//!- restore_state old statefile format
//!--------------------------------------------------------
bool CHybrid::old_restore_state(void)
{
...
//!- hdf_read_BlockFile:
//!------------------------------------------------------------
bool CHybrid::hdf_read_BlockFile(void)
{
	...
	hid_t       dataset_process_id, dataspace_process_id,  memspace_process_id; 
	hid_t       dataset_child_id, dataspace_child_id,memspace_child_id;

	memset(has_child,        0,               8*sizeof(char));
	memset(mpi_process,      0,               8*sizeof(INT32));
	memset(root_mpi_process, 0, num_root_blocks*sizeof(INT32));
	
	hid_t       block_file_id;  /* identifiers */
	hsize_t     count[2];              
	hsize_t     offset[2];

	block_file_id = H5Fopen("State/HDF_State_Blocks.h5", H5F_ACC_RDONLY, H5P_DEFAULT);
	

	//! read time level
   H5LTread_dataset_int(block_file_id,"/TL",&TL);
	...
	//! restore mpi_processes of root array:
	H5LTread_dataset_int(block_file_id,"/root_mpi_process",root_mpi_process);
	...
	dataset_child_id = H5Dopen2(block_file_id, "/has_child", H5P_DEFAULT);
	dataspace_child_id=H5Dget_space(dataset_child_id);
	
	dataset_process_id = H5Dopen2(block_file_id, "/mpi_process", H5P_DEFAULT);
	dataspace_process_id=H5Dget_space(dataset_process_id);
	...
	for(level=0; level<MAX_LEVEL; level++)
	{
		...
		while(temp_Block)
		{
			...
			memspace_process_id = H5Screate_simple (2, count, NULL);  //Get Origin of Block

			H5Sselect_hyperslab (dataspace_process_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
			H5Dread (dataset_process_id, H5T_NATIVE_INT, memspace_process_id,dataspace_process_id, H5P_DEFAULT, mpi_process);
  
			H5Sclose (memspace_process_id);
			
			memspace_child_id = H5Screate_simple (2, count, NULL);  //Get Origin of Block

			H5Sselect_hyperslab (dataspace_child_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
			H5Dread (dataset_child_id, H5T_NATIVE_CHAR, memspace_child_id,dataspace_child_id, H5P_DEFAULT, has_child);
  
			H5Sclose (memspace_child_id);
	...
	H5Dclose(dataset_process_id);
	H5Dclose(dataset_child_id);
...
//!- hdf_read_Field3D:
//!------------------------------------------------------------
void CHybrid::hdf_read_Field3D(INT32 FType, INT32 restorer_mpi_process)
{
	INT32 index=0;
	INT32 level;
	CBlock *temp_Block;
	...
	sprintf(buffer,"State/HDF_State_Fields_p%05d.h5", restorer_mpi_process);

	file_id = H5Fopen(buffer, H5F_ACC_RDONLY, H5P_DEFAULT);
	
//	sprintf(buffer,"State/State_%s_p%05d", Field_Name[FType], );

	{
		...
		sprintf(buffer,"/%s/Level_%d_Data",Field_Name[FType],level);
	
		dataset_id = H5Dopen2(file_id,buffer, H5P_DEFAULT);
		dataspace_id=H5Dget_space(dataset_id);
	
			FieldX = new double[num_nodes_in_block];
			FieldY = new double[num_nodes_in_block];
			FieldZ = new double[num_nodes_in_block];
			...
			if(COMPs_FType[FType]==1) //is scalar field
			{
			
			INT32 i_j_k,i_j_k_DATA;
			INT32 ind[3];
			
				count[0]=num_nodes_in_block;
				count[1]=1;//scalar data
					
					
				offset[0]=num_Blocks_transfered_L[level]*num_nodes_in_block;
				offset[1]=0;
				...
				H5Dread (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldX);
				...
				for(ind[2]=0; ind[2]<BlkNds_Z; ind[2]++)
				    ...
				    for(ind[0]=0; ind[0]<BlkNds_X ; ind[0]++)
				  {
					...
					i_j_k  = ind[0]*BlkNds_Y*BlkNds_Z +ind[1]*BlkNds_Z +ind[2];

					//i_j_k_DATA  = (ind[2])*BlkNds_Y*BlkNds_X+(ind[1])*BlkNds_X+(ind[0]);
					
					VFieldX[i_j_k]=FieldX[index];
			...
			if(COMPs_FType[FType]==3) //is vector field
			{
				...
				VFieldX = temp_Block->Field_Type[FType];
				...
				H5Dread (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldY);
				...
				H5Dread (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldZ);
				...
				for(ind[2]=0; ind[2]<BlkNds_Z; ind[2]++)
					...
					for(ind[0]=0; ind[0]<BlkNds_X ; ind[0]++)
				  {
					...
					VFieldX[i_j_k]=FieldX[index];
					VFieldY[i_j_k]=FieldY[index];
					VFieldZ[i_j_k]=FieldZ[index];
...
//!- hdf_read_Particle:
//!------------------------------------------------------------
void CHybrid::hdf_read_Particle(INT32 restorer_mpi_process)
{
	...
	hid_t      HDF_Particle_File; 
	hid_t       dataset_num_id,dataspace_num_id,  memspace_num_id; 
	hid_t       particle_v_dataset_id,particle_v_dataspace_id,particle_v_memspace_id; 
	hid_t       particle_coords_dataset_id,particle_coords_dataspace_id,particle_coords_memspace_id; 
	hid_t       particle_weight_dataset_id,particle_weight_dataspace_id,particle_weight_memspace_id; 
	...
	sprintf(buffer,"State/HDF_State_Particles_p%05d.h5", restorer_mpi_process);
	HDF_Particle_File = H5Fopen(buffer, H5F_ACC_RDONLY, H5P_DEFAULT);
	...
	for(INT32 species=0; species<num_Charged_Species; species++)
	{
		...
		// 	
			for(level=0; level<=MAX_LEVEL; level++)
			{
				...
				while(temp_Block)
				{
					...
					if(restorer_mpi_process == temp_Block->responsible_mpi_process)
					{
					//log_file << " restoring Particle of species " << species << " in level"<<level <<" of block "<< temp_Block->Block_Nr<<" in process "<< temp_Block->responsible_mpi_process<<endl;
					  
					sprintf(buffer,"/Species_%d/Level_%d/Block_%lld/num_MPiC_Block", species,level,temp_Block->Block_Nr);
					dataset_num_id = H5Dopen2(HDF_Particle_File ,buffer, H5P_DEFAULT);
					dataspace_num_id=H5Dget_space(dataset_num_id);
		...
		// 				if(!temp_Block->child_array)
						{					  
						  ...
						  if (convert_TRACK_PARTICLE_State==0)
						  {
						      ...
						      for(INT32 cell=0; cell < num_nodes_in_block; cell++)
						      {
								...
								memspace_num_id = H5Screate_simple (2, count, NULL); 
								H5Sselect_hyperslab (dataspace_num_id, H5S_SELECT_SET, offset, NULL,count, NULL);
								H5Dread (dataset_num_id, H5T_NATIVE_INT, memspace_num_id,dataspace_num_id, H5P_DEFAULT, &temp_Block->num_MPiC[species][cell]);
				  				H5Sclose (memspace_num_id);
												     	
				
							      //! increase size/set new size if required
							      if(temp_Block->size_of_pArray[species][cell] < temp_Block->num_MPiC[species][cell])
							      temp_Block->resize_pArray(species, cell, int( min_pArray_Size_factor *temp_Block->num_MPiC[species][cell]));

							//log_file<<cell<<" State "<<convert_TRACK_PARTICLE_State<<" "<<temp_Block->num_MPiC[species][cell]<<endl;
							
							if (temp_Block->num_MPiC[species][cell]>0) { //only execute if there are particles in macrocell
								

								sprintf(buffer,"/Species_%d/Level_%d/Block_%lld/Coords/Cell_%d_particle_coords", species,level,temp_Block->Block_Nr,cell);
								 particle_coords_dataset_id = H5Dopen2(HDF_Particle_File, buffer, H5P_DEFAULT);
								particle_coords_dataspace_id=H5Dget_space(particle_coords_dataset_id);
		
								sprintf(buffer,"/Species_%d/Level_%d/Block_%lld/V/Cell_%d_particle_v", species,level,temp_Block->Block_Nr,cell);
								particle_v_dataset_id = H5Dopen2(HDF_Particle_File, buffer, H5P_DEFAULT);
								particle_v_dataspace_id = H5Dget_space(particle_v_dataset_id);			
								
								sprintf(buffer,"/Species_%d/Level_%d/Block_%lld/Weight/Cell_%d_particle_weight", species,level,temp_Block->Block_Nr,cell);
								particle_weight_dataset_id = H5Dopen2(HDF_Particle_File, buffer, H5P_DEFAULT);
								particle_weight_dataspace_id = H5Dget_space(particle_weight_dataset_id);
								
								
		
							for(INT32 part_in_cell=0;part_in_cell<temp_Block->num_MPiC[species][cell];part_in_cell++) 
								{
								offset[0]=part_in_cell;//count variable for lines
								offset[1]=0;
								
								count[0]=1;
								count[1]=3;	//v and coords are 3D vector
								
								particle_coords_memspace_id = H5Screate_simple (2, count, NULL);  //Get paarticle coords

								H5Sselect_hyperslab (particle_coords_dataspace_id, H5S_SELECT_SET, offset, NULL,count, NULL);
							  
								H5Dread (particle_coords_dataset_id, H5T_NATIVE_DOUBLE, particle_coords_memspace_id,particle_coords_dataspace_id, H5P_DEFAULT, temp_Block->pArray[species][cell][part_in_cell].rel_r);
								
								H5Sclose (particle_coords_memspace_id);
								
								particle_v_memspace_id = H5Screate_simple (2, count, NULL);  //Get paarticle v

								H5Sselect_hyperslab (particle_v_dataspace_id, H5S_SELECT_SET, offset, NULL,count, NULL);
							  
								H5Dread (particle_v_dataset_id, H5T_NATIVE_DOUBLE, particle_v_memspace_id,particle_v_dataspace_id, H5P_DEFAULT, temp_Block->pArray[species][cell][part_in_cell].v);
								
								H5Sclose (particle_v_memspace_id);
								
								count[0]=1;
								count[1]=1;	//weight is scalar
								
								particle_weight_memspace_id = H5Screate_simple (2, count, NULL);  //Get paarticle weight

								H5Sselect_hyperslab (particle_weight_dataspace_id, H5S_SELECT_SET, offset, NULL,count, NULL);
							  
								H5Dread (particle_weight_dataset_id, H5T_NATIVE_FLOAT, particle_weight_memspace_id,particle_weight_dataspace_id, H5P_DEFAULT, &temp_Block->pArray[species][cell][part_in_cell].weight);
								
								H5Sclose (particle_weight_memspace_id);

								}

							H5Dclose(particle_coords_dataset_id);
							H5Dclose(particle_v_dataset_id);
							H5Dclose(particle_weight_dataset_id);
							}
							      num_total_particles += temp_Block->num_MPiC[species][cell];
							      num_total_particles_in_L[temp_Block->RLevel] += temp_Block->num_MPiC[species][cell];
							      num_particle_processed_L[temp_Block->RLevel] += temp_Block->num_MPiC[species][cell];
						   ...
						   #ifdef TRACK_PARTICLE
						  if (convert_TRACK_PARTICLE_State==1)
						  { 
						    ...
						    for(INT32 cell=0; cell < num_nodes_in_block; cell++)
						    {
							    ...
							    if(temp_Block->size_of_pArray[species][cell]>0)
							    {
								    ORDINARY_particle_Array = new ORDINARY_particle[temp_Block->size_of_pArray[species][cell]];
							...
							for(INT32 part_in_cell=0;part_in_cell<temp_Block->num_MPiC[species][cell];part_in_cell++) 
								...
								H5Dread (particle_coords_dataset_id, H5T_NATIVE_DOUBLE, particle_coords_memspace_id,particle_coords_dataspace_id, H5P_DEFAULT, ORDINARY_particle_Array[part_in_cell].rel_r);
								...
								H5Dread (particle_v_dataset_id, H5T_NATIVE_DOUBLE, particle_v_memspace_id,particle_v_dataspace_id, H5P_DEFAULT, ORDINARY_particle_Array[part_in_cell].v);
								...
								H5Dread (particle_weight_dataset_id, H5T_NATIVE_FLOAT, particle_weight_memspace_id,particle_weight_dataspace_id, H5P_DEFAULT, &ORDINARY_particle_Array[part_in_cell].weight);
							...
							H5Dclose(particle_weight_dataset_id);
								    ...
								    for(INT32 part=0; part<temp_Block->size_of_pArray[species][cell]; part++)
								    {
						    
						    
									    temp_Block->pArray[species][cell][part].rel_r[0] = ORDINARY_particle_Array[part].rel_r[0];
									    temp_Block->pArray[species][cell][part].rel_r[1] = ORDINARY_particle_Array[part].rel_r[1];
									    temp_Block->pArray[species][cell][part].rel_r[2] = ORDINARY_particle_Array[part].rel_r[2];
						    
									    temp_Block->pArray[species][cell][part].v[0] = ORDINARY_particle_Array[part].v[0];
									    temp_Block->pArray[species][cell][part].v[1] = ORDINARY_particle_Array[part].v[1];
									    temp_Block->pArray[species][cell][part].v[2] = ORDINARY_particle_Array[part].v[2];
						    
									    temp_Block->pArray[species][cell][part].weight = ORDINARY_particle_Array[part].weight;
						    
						    
									    temp_Block->pArray[species][cell][part].number   =  -1;
									    temp_Block->pArray[species][cell][part].exchange =  -1;
						  ...
						  if (convert_TRACK_PARTICLE_State==2)
						  {
							...
							for(INT32 cell=0; cell < num_nodes_in_block; cell++)
							{
								...
								H5Sclose (memspace_num_id);
									...
									TRACK_particle_Array = new TRACK_particle[temp_Block->size_of_pArray[species][cell]];
							...
							for(INT32 part_in_cell=0;part_in_cell<temp_Block->num_MPiC[species][cell];part_in_cell++) 
								...
								H5Dread (particle_coords_dataset_id, H5T_NATIVE_DOUBLE, particle_coords_memspace_id,particle_coords_dataspace_id, H5P_DEFAULT, TRACK_particle_Array[part_in_cell].rel_r);
								...
								H5Dread (particle_v_dataset_id, H5T_NATIVE_DOUBLE, particle_v_memspace_id,particle_v_dataspace_id, H5P_DEFAULT, TRACK_particle_Array[part_in_cell].v);
								...
								H5Dread (particle_weight_dataset_id, H5T_NATIVE_FLOAT, particle_weight_memspace_id,particle_weight_dataspace_id, H5P_DEFAULT, &TRACK_particle_Array[part_in_cell].weight);
							...
							H5Dclose(particle_weight_dataset_id);
									...
									for(INT32 part=0; part<temp_Block->size_of_pArray[species][cell]; part++)
									{
										...
										temp_Block->pArray[species][cell][part].rel_r[0] = TRACK_particle_Array[part].rel_r[0];
										temp_Block->pArray[species][cell][part].rel_r[1] = TRACK_particle_Array[part].rel_r[1];
										temp_Block->pArray[species][cell][part].rel_r[2] = TRACK_particle_Array[part].rel_r[2];
							
										temp_Block->pArray[species][cell][part].v[0] = TRACK_particle_Array[part].v[0];
										temp_Block->pArray[species][cell][part].v[1] = TRACK_particle_Array[part].v[1];
										temp_Block->pArray[species][cell][part].v[2] = TRACK_particle_Array[part].v[2];
							
										temp_Block->pArray[species][cell][part].weight = TRACK_particle_Array[part].weight;
...
//!- hdf_restore_state
//!--------------------------------------------------------
bool CHybrid::hdf_restore_state(void)
{
	...
	log_file << endl << " Searching for HDF5 state files ...     ";
	
	
	//check if at least statefiles for one process are available
	
      if(!FileExists("State/HDF_State_Blocks.h5")) return false;
      if(!FileExists("State/HDF_State_Particles_p00000.h5")) return false;
      if(!FileExists("State/HDF_State_Fields_p00000.h5")) return false;
	...
	for(INT32 restore_process_id = restore_start_id;  restore_process_id<restore_end_id; restore_process_id++)
	{
		...
		//! - carefull, id_BTotal and id_BEven may have the same name
		//! - restore of id_BTotal should not be required
		hdf_read_Field3D(  id_BEven, restore_process_id);
		hdf_read_Field3D( id_EField, restore_process_id);
                hdf_read_Field3D( id_PEtotal, restore_process_id);

		hdf_read_Field3D(  id_PhiDC, restore_process_id);
		
		for(INT32 average_field=0; average_field<num_average_fields; average_field++)
		hdf_read_Field3D(id_average_Field1 +average_field, restore_process_id);
			
		hdf_read_Particle(restore_process_id);
...
//!- hdf_write_BlockFile:
//!------------------------------------------------------------
void CHybrid::hdf_write_BlockFile(void)
{
	...
	hid_t       dataset_origin_id, dataspace_origin_id,  memspace_origin_id; 
	hid_t       dataset_end_id, dataspace_end_id,  memspace_end_id; 
	hid_t       dataset_level_id, dataspace_level_id,  memspace_level_id; 
	hid_t       dataset_process_id, dataspace_process_id,  memspace_process_id; 
	hid_t       dataset_cells_id, dataspace_cells_id,memspace_cells_id;
	hid_t       dataset_child_id, dataspace_child_id,memspace_child_id;
	hid_t    plist_id_1,plist_id_2;
   ...
   file_id = H5Fcreate("State/HDF_State_Blocks.h5", H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);	
	...
	//! write update TL into Blockfile	
	H5LTmake_dataset_int(file_id,"/TL",1,hdf_dims,&TL);

	//! Size of simulation box
	H5LTmake_dataset_double(file_id,"/LX",1,hdf_dims,&LX);
	H5LTmake_dataset_double(file_id,"/LY",1,hdf_dims,&LY);
	H5LTmake_dataset_double(file_id,"/LZ",1,hdf_dims,&LZ);
	
	double tmp;
	
	
	tmp=factor_scale_mesh*(-Box_Origin[0]);
	
	//! Min extent of Sim-Box
	H5LTmake_dataset_double(file_id,"/Min_X",1,hdf_dims,&tmp);
	
	tmp=factor_scale_mesh*(-Box_Origin[1]);
	
	H5LTmake_dataset_double(file_id,"/Min_Y",1,hdf_dims,&tmp);
	
	tmp=factor_scale_mesh*(-Box_Origin[2]);
	
	H5LTmake_dataset_double(file_id,"/Min_Z",1,hdf_dims,&tmp);
	
	
	tmp=factor_scale_mesh*(LX-Box_Origin[0]);
	
	//! Max extent of Sim-Box
	H5LTmake_dataset_double(file_id,"/Max_X",1,hdf_dims,&tmp);
	
	tmp=factor_scale_mesh*(LY-Box_Origin[1]);
	
	H5LTmake_dataset_double(file_id,"/Max_Y",1,hdf_dims,&tmp);
	
	tmp=factor_scale_mesh*(LZ-Box_Origin[2]);
	
	H5LTmake_dataset_double(file_id,"/Max_Z",1,hdf_dims,&tmp);
	
	//! Number of Root Blocks in Szenario
	H5LTmake_dataset_int(file_id,"/RB_X",1,hdf_dims,&RB_X);
	H5LTmake_dataset_int(file_id,"/RB_Y",1,hdf_dims,&RB_Y);
	H5LTmake_dataset_int(file_id,"/RB_Z",1,hdf_dims,&RB_Z);
	
	//! write max ref_level into Blockfile	
	H5LTmake_dataset_int(file_id,"/Max_Level",1,hdf_dims,&MAX_LEVEL);
	
	//! Number of Root Blocks in Szenario
	H5LTmake_dataset_int(file_id,"/BlkNds_X",1,hdf_dims,&BlkNds_X);
	H5LTmake_dataset_int(file_id,"/BlkNds_Y",1,hdf_dims,&BlkNds_Y);
	H5LTmake_dataset_int(file_id,"/BlkNds_Z",1,hdf_dims,&BlkNds_Z);
	
	H5LTmake_dataset_int(file_id,"/Num_Ion_Species",1,hdf_dims,&num_Charged_Species);
	
	H5LTmake_dataset_int(file_id,"/mpi_num_processes",1,hdf_dims,&mpi_num_processes);
	...
	for(level=0; level<=MAX_LEVEL; level++)
	{
		...
		//! Number of total Blocks in Szenario
	H5LTmake_dataset_float(file_id,"/Factor_Scale_Mesh",1,hdf_dims,&factor_scale_mesh);	
	
	
	hdf_dims[0] = max_blocks; //length of data array
	hdf_dims[1] = 3; //data is 3D vector
	
	cdims[0] = (int)(hdf_dims[0]/chunk_factor)+1;
    cdims[1] = 1;
	
	plist_id_1  = H5Pcreate (H5P_DATASET_CREATE);
	H5Pset_chunk (plist_id_1, 2, cdims);
	H5Pset_deflate (plist_id_1, 9);
	
	
	plist_id_2  = H5Pcreate (H5P_DATASET_CREATE);
	H5Pset_chunk (plist_id_2, 2, cdims);
	H5Pset_deflate (plist_id_2, 9);
	
	dataspace_origin_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_origin_id = H5Dcreate2(file_id, "/Block_Origin", H5T_NATIVE_DOUBLE, dataspace_origin_id, H5P_DEFAULT, plist_id_1, H5P_DEFAULT);
	
	dataspace_end_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_end_id = H5Dcreate2(file_id, "/Block_End", H5T_NATIVE_DOUBLE, dataspace_end_id, H5P_DEFAULT, plist_id_2, H5P_DEFAULT);
	
	
	hdf_dims[0] = max_blocks; //length of data array
	hdf_dims[1] = 1; //data is 1D vector
		
	dataspace_level_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_level_id = H5Dcreate2(file_id, "/Block_Level", H5T_NATIVE_INT, dataspace_level_id, H5P_DEFAULT,  H5P_DEFAULT, H5P_DEFAULT);
	
	dataspace_process_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_process_id = H5Dcreate2(file_id, "/Block_Process", H5T_NATIVE_INT, dataspace_process_id, H5P_DEFAULT,  H5P_DEFAULT, H5P_DEFAULT);
	
	//! store mpi_processes of root array and write Blocks to State File:
	for(INT32 blk=0; blk<num_root_blocks; blk++){
	root_mpi_process[blk] = Root_Block_Array[blk].responsible_mpi_process;
	}
...
INT64 blk=0;
	...
	for(level=0; level<=MAX_LEVEL; level++)
	{
		...
		sprintf(filename,"/Level_%d", level);
		group_id = H5Gcreate2(file_id, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		H5Gclose (group_id);
		
		hdf_dims[0] = 1;
		hdf_dims[1] = 3;
		
		sprintf(filename,"/Level_%d/delta_of_L", level);
		H5LTmake_dataset_double(file_id,filename,2,hdf_dims,delta_of_L[level]);
	...
	block_end[0] = factor_scale_mesh * r_vec[0];

	temp_Block->intern2normedCoords(r_vec, cell_intern_r,ind);
	block_end[1] = factor_scale_mesh * r_vec[1];

	temp_Block->intern2normedCoords(r_vec, cell_intern_r,ind);
	block_end[2] = factor_scale_mesh * r_vec[2];


	//write relative origin coords
				
	memspace_origin_id = H5Screate_simple (2, count, NULL); 

	H5Sselect_hyperslab (dataspace_origin_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
	H5Dwrite (dataset_origin_id, H5T_NATIVE_DOUBLE, memspace_origin_id,dataspace_origin_id, H5P_DEFAULT, block_end);
  
	H5Sclose (memspace_origin_id);
	
	
	ind[0]=(BlkNds_X-hide_pGN)-1;
	ind[1]=(BlkNds_Y-hide_pGN)-1;
	ind[2]=(BlkNds_Z-hide_pGN)-1;
	...
	memspace_end_id = H5Screate_simple (2, count, NULL); 

	H5Sselect_hyperslab (dataspace_end_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
	H5Dwrite (dataset_end_id, H5T_NATIVE_DOUBLE, memspace_end_id,dataspace_end_id, H5P_DEFAULT, block_end);
  
	H5Sclose (memspace_end_id);	
	
	count[0]=1;
	count[1]=1;	
	
	memspace_level_id = H5Screate_simple (2, count, NULL); 

	H5Sselect_hyperslab (dataspace_level_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
	H5Dwrite (dataset_level_id, H5T_NATIVE_INT, memspace_level_id,dataspace_level_id, H5P_DEFAULT, &(temp_Block->RLevel));
  
	H5Sclose (memspace_level_id);
	
	
	memspace_process_id = H5Screate_simple (2, count, NULL); 

	H5Sselect_hyperslab (dataspace_process_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
	H5Dwrite (dataset_process_id, H5T_NATIVE_INT, memspace_process_id,dataspace_process_id, H5P_DEFAULT, &(temp_Block->responsible_mpi_process));
  
	H5Sclose (memspace_process_id);
	
	hdf_dims[0] = num_nodes_in_block; //length of data array
	hdf_dims[1] = 3; //data is 3D vector
	
	sprintf(filename,"Level_%d/Node_Coords_of_Block_%d",level,blk);
	
	dataspace_cells_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_cells_id = H5Dcreate2(file_id,filename , H5T_NATIVE_DOUBLE, dataspace_cells_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		  ...
		  //if(temp_Block->num_children<8)
		  {
			...
			for(ind[0]=0; ind[0]<BlkNds_X; ind[0]++)
			  for(ind[1]=0; ind[1]<BlkNds_Y; ind[1]++)
				...
				offset[0]= (ind[2])*BlkNds_Y*BlkNds_X+(ind[1])*BlkNds_X+(ind[0]);
				offset[1]=0;
	
				count[0]=1;
				count[1]=3;	
			
				//write abs cells coords
				
				memspace_cells_id = H5Screate_simple (2, count, NULL); 

				H5Sselect_hyperslab (dataspace_cells_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
				H5Dwrite (dataset_cells_id, H5T_NATIVE_DOUBLE, memspace_cells_id,dataspace_cells_id, H5P_DEFAULT, block_end);
  
				H5Sclose (memspace_cells_id);
	...
	H5Dclose(dataset_cells_id);

	/* Terminate access to the data space. */ 
	H5Sclose(dataspace_cells_id);
	...
	/* End access to the dataset and release resources used by it. */
	H5Dclose(dataset_origin_id);

	/* Terminate access to the data space. */ 
	H5Sclose(dataspace_origin_id);
	
	/* End access to the dataset and release resources used by it. */
	H5Dclose(dataset_end_id);

	/* Terminate access to the data space. */ 
	H5Sclose(dataspace_end_id);
	
	/* End access to the dataset and release resources used by it. */
	H5Dclose(dataset_level_id);

	/* Terminate access to the data space. */ 
	H5Sclose(dataspace_level_id);
	...
	hdf_dims[0] = num_root_blocks; //set new dims
	
	H5LTmake_dataset_int(file_id,"/root_mpi_process",1,hdf_dims,root_mpi_process);
	
	hdf_dims[0] = max_blocks; //length of data array
	hdf_dims[1] = 8; //8 elements per oct
	...
	dataspace_child_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_child_id = H5Dcreate2(file_id, "/has_child", H5T_NATIVE_CHAR, dataspace_child_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	
	dataspace_process_id = H5Screate_simple(2, hdf_dims, NULL);
	dataset_process_id = H5Dcreate2(file_id, "/mpi_process", H5T_NATIVE_INT, dataspace_process_id, H5P_DEFAULT, plist_id_1, H5P_DEFAULT);
	...
	for(level=0; level<MAX_LEVEL; level++)
	{
		...
		while(temp_Block)
		{
			...
			memset(has_child,   0, 8*sizeof(char)); //use char as stand-in for bool because char is just 8bit and bool ist just a renamed int32
			memset(mpi_process, 0, 8*sizeof(INT32));
			...
			for(INT32 oct=0; oct<8; oct++)
			 if(temp_Block->child_array[oct])
			 {
				...
				memspace_child_id = H5Screate_simple (2, count, NULL); 

				H5Sselect_hyperslab (dataspace_child_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
				H5Dwrite (dataset_child_id, H5T_NATIVE_CHAR, memspace_child_id,dataspace_child_id, H5P_DEFAULT, has_child);
				...
				H5Dwrite (dataset_process_id, H5T_NATIVE_INT, memspace_process_id,dataspace_process_id, H5P_DEFAULT, mpi_process);
	...
	/* Terminate access to the data space. */ 
	H5Sclose(dataspace_process_id);
	
	/* End access to the dataset and release resources used by it. */
	H5Dclose(dataset_process_id);
	
		/* Terminate access to the data space. */ 
	H5Sclose(dataspace_child_id);
...
//!- hdf_write_Particle:
//!------------------------------------------------------------
void CHybrid::hdf_write_Particle()
{
		hid_t       dataset_num_id,dataspace_num_id,  memspace_num_id; 
		hid_t       dataset_coords_id, dataspace_coords_id,  memspace_coords_id; 
		hid_t       dataset_v_id, dataspace_v_id,  memspace_v_id; 
		hid_t       dataset_weight_id, dataspace_weight_id,  memspace_weight_id; 
		hid_t	 group_id;
		hid_t       HDF_Particle_File;
		hid_t    plist_id_1,plist_id_2,plist_id_3;
	        hsize_t  	cdims[2]; //chunk size used for compression
		hsize_t     hdf_dims[2];//max 3D for HDF5 file
		hsize_t     count[2];              /* size of the hyperslab in the file */
		hsize_t     offset[2];
	...
	log_file << endl << " hdf writing Particle ..." << endl << endl;
	...
	sprintf(filename,"State/HDF_State_Particles_p%05d.h5",mpi_myRank);
	HDF_Particle_File = H5Fopen(filename, H5F_ACC_RDWR, H5P_DEFAULT);
   ...
   for(INT32 species=0; species<num_Charged_Species; species++)
   {
		...
		sprintf(filename,"/Species_%d", species);
		group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		H5Gclose (group_id);
		...
		log_file << " -----------------------------------" << endl;
		log_file << " <<<<<<<<<<<<<Writing Level "<< level << ">>>>>>>>>>>>>>" << endl;
		
		sprintf(filename,"/Species_%d/Level_%d", species,level);
		group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
		H5Gclose (group_id);
		...
		if(mpi_myRank == temp_Block->responsible_mpi_process)
			...
			sprintf(filename,"/Species_%d/Level_%d/Block_%d", species,level,temp_Block->Block_Nr);
			group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			H5Gclose (group_id);

			sprintf(filename,"/Species_%d/Level_%d/Block_%d/Coords", species,level,temp_Block->Block_Nr);
			group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			H5Gclose (group_id);
			
			sprintf(filename,"/Species_%d/Level_%d/Block_%d/V", species,level,temp_Block->Block_Nr);
			group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			H5Gclose (group_id);
			
			sprintf(filename,"/Species_%d/Level_%d/Block_%d/Weight", species,level,temp_Block->Block_Nr);
			group_id = H5Gcreate2(HDF_Particle_File, filename, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			H5Gclose (group_id);
			
			
			hdf_dims[0] = num_nodes_in_block; //length of data array
			hdf_dims[1] = 1; //data is scalar
			...
			dataspace_num_id = H5Screate_simple(2, hdf_dims, NULL);	
			sprintf(filename,"/Species_%d/Level_%d/Block_%d/num_MPiC_Block", species,level,temp_Block->Block_Nr);
			dataset_num_id = H5Dcreate2(HDF_Particle_File, filename, H5T_NATIVE_INT, dataspace_num_id, H5P_DEFAULT, plist_id_1, H5P_DEFAULT);
				...
				H5Dwrite (dataset_num_id, H5T_NATIVE_INT, memspace_num_id,dataspace_num_id, H5P_DEFAULT, (temp_Block->num_MPiC[species]+cell));
  
				H5Sclose (memspace_num_id);
	
				//write particle coords and velocity for each particle in cell
				hdf_dims[0] = temp_Block->num_MPiC[species][cell]; //length of data array
				hdf_dims[1] = 3; //data is 3D vector
				...
				plist_id_3  = H5Pcreate (H5P_DATASET_CREATE);
				H5Pset_chunk (plist_id_3, 2, cdims);

				H5Pset_deflate (plist_id_3, 9); 
								
			
	
				dataspace_coords_id = H5Screate_simple(2, hdf_dims, NULL);
				sprintf(filename,"/Species_%d/Level_%d/Block_%d/Coords/Cell_%d_particle_coords", species,level,temp_Block->Block_Nr,cell);
				dataset_coords_id = H5Dcreate2(HDF_Particle_File, filename, H5T_NATIVE_DOUBLE, dataspace_coords_id, H5P_DEFAULT, plist_id_2, H5P_DEFAULT);
	
	
				dataspace_v_id = H5Screate_simple(2, hdf_dims, NULL);
				sprintf(filename,"/Species_%d/Level_%d/Block_%d/V/Cell_%d_particle_v", species,level,temp_Block->Block_Nr,cell);
				dataset_v_id = H5Dcreate2(HDF_Particle_File, filename, H5T_NATIVE_DOUBLE, dataspace_v_id, H5P_DEFAULT, plist_id_3, H5P_DEFAULT);
				
				
				dataspace_weight_id = H5Screate_simple(2, hdf_dims, NULL);
				sprintf(filename,"/Species_%d/Level_%d/Block_%d/Weight/Cell_%d_particle_weight", species,level,temp_Block->Block_Nr,cell);
				dataset_weight_id = H5Dcreate2(HDF_Particle_File, filename, H5T_NATIVE_FLOAT, dataspace_weight_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			...
			for(INT32 part_in_cell=0;part_in_cell<temp_Block->num_MPiC[species][cell];part_in_cell++) 
				...
				memspace_coords_id = H5Screate_simple (2, count, NULL); 

				H5Sselect_hyperslab (dataspace_coords_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
				H5Dwrite (dataset_coords_id, H5T_NATIVE_DOUBLE, memspace_coords_id,dataspace_coords_id, H5P_DEFAULT, (temp_Block->pArray[species][cell][part_in_cell].rel_r));
  
				H5Sclose (memspace_coords_id);
				
				
				
				//write particle velocity
				
				memspace_v_id = H5Screate_simple (2, count, NULL); 

				H5Sselect_hyperslab (dataspace_v_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
				H5Dwrite (dataset_v_id, H5T_NATIVE_DOUBLE, memspace_v_id,dataspace_v_id, H5P_DEFAULT,  (temp_Block->pArray[species][cell][part_in_cell].v));
  
				H5Sclose (memspace_v_id);
				
				
				
				//write how many "real" ions are represented by macro particle
	
				count[0]=1;
				count[1]=1;
				
				memspace_weight_id = H5Screate_simple (2, count, NULL); 

				H5Sselect_hyperslab (dataspace_weight_id, H5S_SELECT_SET, offset, NULL,count, NULL);
  
				H5Dwrite (dataset_weight_id, H5T_NATIVE_FLOAT, memspace_weight_id,dataspace_weight_id, H5P_DEFAULT,  &(temp_Block->pArray[species][cell][part_in_cell].weight));
  
				H5Sclose (memspace_weight_id);

				}
				
				// End access to the dataset and release resources used by it. 
				H5Dclose(dataset_coords_id);

				// Terminate access to the data space.  
				H5Sclose(dataspace_coords_id);
				...
				// End access to the dataset and release resources used by it. 
				H5Dclose(dataset_weight_id);

				// Terminate access to the data space. 
				H5Sclose(dataspace_weight_id);
...
//!- hdf_write_Field3D:
//!------------------------------------------------------------
void CHybrid::hdf_write_Field3D(INT32 FType)
{
	...
	sprintf(buffer,"State/HDF_State_Fields_p%05d.h5", mpi_myRank);
	...
	hdf_dims[0] = blocks_in_level*num_nodes_in_block; //length of data array
	
	if(COMPs_FType[FType]==1){ //is scalar field
	hdf_dims[1] = 1; //data is 1D scalar
	}
	if(COMPs_FType[FType]==3){ //is scalar field
	hdf_dims[1] = 3; //data is 3D vector
	}
	...
	sprintf(buffer,"/%s/Level_%d_Data",Field_Name[FType],level);
	dataset_id = H5Dcreate2(file_id, buffer, H5T_NATIVE_DOUBLE, dataspace_id, H5P_DEFAULT, plist_id_1, H5P_DEFAULT);
			...
			for(ind[2]=0; ind[2]<BlkNds_Z; ind[2]++)
				for(ind[1]=0; ind[1]<BlkNds_Y; ind[1]++)
					for(ind[0]=0; ind[0]<BlkNds_X ; ind[0]++)
				  {
					...
					//i_j_k_DATA  =ind[2]*num_nodes_in_block*num_nodes_in_block+ind[1]*num_nodes_in_block+ind[0];
					
					FieldX[index] = temp_Block->Field_Type[FType][i_j_k];
					...
					H5Dwrite (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldX);
			...
			if(COMPs_FType[FType]==3) //is vector field
			{
				...
				for(ind[2]=0; ind[2]<BlkNds_Z; ind[2]++)
					...
					for(ind[0]=0; ind[0]<BlkNds_X ; ind[0]++)
				  {
					...
					FieldX[index] = VFieldX[i_j_k];
					FieldY[index] = VFieldY[i_j_k];
					FieldZ[index] = VFieldZ[i_j_k];
				  ...
				  index=0;
					...
					H5Dwrite (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldY);
					...
					H5Dwrite (dataset_id, H5T_NATIVE_DOUBLE, memspace_id,dataspace_id, H5P_DEFAULT, FieldZ);
...
//!  SYNCHRONIZATION ERRORS !!!
//!--------------------------------------------------------
void CHybrid::hdf_save_state(void)
{
	...
	log_file << " Saving state to HDF5 file..." << endl;
	log_file << " backing up last state ... ";
	synchronize_allProcesses_MPI();	
	...
	sprintf(buffer,"mkdir -p Last_State");
	system(buffer);
	...
	/* Create a new HDF5 file for all fields of each process using default properties. */
	sprintf(filename,"State/HDF_State_Fields_p%05d.h5",mpi_myRank);
	file_id = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	H5Fclose(file_id);	//! close HDF5 File	
	...
	//! write density and velocity state files
	hdf_write_Field3D(id_rho_np1);
	
	for(int species=0; species<num_Charged_Species; species++)
	{
	  hdf_write_Field3D(id_rhoSpecies1 +species);
	  
	  collect_Ui_vth2_of_species(species, id_UI_Species1 +species, 0, noVTH2);
	  hdf_write_Field3D(id_UI_Species1 +species);
	}
	
	hdf_write_Field3D(id_UI_plus);
	
	//! - carefull, id_BTotal and id_BEven may have the same names
	//! - id_BTotal should not be required
	//    write_Field3D(id_BTotal,TL,true);
	hdf_write_Field3D(id_BEven);

	hdf_write_Field3D(id_EField);
        
        hdf_write_Field3D(id_PEtotal);

	if(!TestParticle_Simulation){
	hdf_write_Field3D(id_PhiDC);
	}

	if(!TestParticle_Simulation){
	for(INT32 average_field=0; average_field<num_average_fields; average_field++)
	{
	hdf_write_Field3D(id_average_Field1 +average_field);
	}
	}
	
	
	hdf_write_Particle();
	synchronize_allProcesses_MPI();	//synchronize Processes after writing is done
	
	
	time_t rawtime;
	time ( &rawtime );
	log_file << " Writing finished at: " <<  ctime (&rawtime)<< endl << endl;
