...
void CBlock::accelerate_particle(INT32 id_oct)
{
    ...
    D_REAL M = 0; // Mass of the gravitational source
    D_REAL r1[3] = {0.,0.,0.}; // Position point 1
    D_REAL r2[3] = {0.,0.,0.}; // Position point 2
    D_REAL loc_r[3] = {0.,0.,0.}; // Dummy local location
    D_REAL gM; // Gravitation acceleration due to M
    D_REAL w[3] = {0.,0.,0.}; // Rotation of the ref. frame
	...
	for(INT32 j = oct_start[1]; j < oct_end[1]; j++)
		...
		for(INT32 species=0; species<num_Charged_Species; species++)
			...
			ind[1]=j;
			ind[2]=k;
			intern2normedCoords(r2, loc_r, ind);

			//! Primary object//
			for(int m=0; m<3; m++) r1[m] = 0;
			M = M_Obstacle*SI_m0;
			gM = G*M/(SI_x0*SI_x0)/SI_a0;
			apply_Gravitation(r1, r2, gM, v, dt_particle_of_L[RLevel]);
			if(use_Star==true) {
			    //! Secondary obstacle//
			    for(int m=0; m<3; m++) r1[m] = Position_Star[m];
			    M = M_Star*SI_m0;
			    gM = G*M/(SI_x0*SI_x0)/SI_a0;
			    apply_Gravitation(r1, r2, gM, v, dt_particle_of_L[RLevel]);
			}
			...
			ind[1]=j;
			ind[2]=k;
			intern2normedCoords(r, loc_r, ind);
			apply_RotationEffects(r, w_Frame, v, dt_particle_of_L[RLevel]);
#endif
