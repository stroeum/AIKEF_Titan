...
void CHybrid::init(void)
{
	...
	if(extended_upstream_boundary_conditions)
        {
                init_extended_upstream_boundary_conditions();
        }
...
//! init_extended_upstream_boundary_conditions
//!--------------------------------------------------------
void CHybrid::init_extended_upstream_boundary_conditions(void)
{
        //! Before entering the time loop_start_comp
        //! Calculate for the velocity,numberdensity_sw,numberdensity_ci        
        //! for roughly each mesh point into an array, 
        //! should allow a fast request
        
        //! Min R = 0
        //! Max R = sqrt((LY/2)^2+(LZ/2)^2)
        D_REAL max_radius = sqrt(Box_Origin[1]*Box_Origin[1]+Box_Origin[2]*Box_Origin[2]);
        
        //! init array (length*3, since array store velocity, numberdensity_sw and numberdensity_ci)
        INT32 length_precalc_array = BlkNds_Y*RB_Y*BlkNds_Z*RB_Z;
        eubc_precalc_values_velocity = new D_REAL [length_precalc_array];
        eubc_precalc_values_numberdensity_sw = new D_REAL [length_precalc_array];
        eubc_precalc_values_numberdensity_ci = new D_REAL [length_precalc_array];
        memset(eubc_precalc_values_velocity,0,length_precalc_array*sizeof(D_REAL));
        memset(eubc_precalc_values_numberdensity_sw,0,length_precalc_array*sizeof(D_REAL));
        memset(eubc_precalc_values_numberdensity_ci,0,length_precalc_array*sizeof(D_REAL));

        //! calc mean neutral mass
        eubc_mean_neutral_mass=0;
        for(int i=0; i<num_Neutral_Species;i++)
                eubc_mean_neutral_mass +=Neutral_Masses[i];
        eubc_mean_neutral_mass = (eubc_mean_neutral_mass/num_Neutral_Species)*m_p;
//         log_file << "Mean New Ion Mass " << eubc_mean_neutral_mass/m_p << endl;
        
        //! Set Neutral Density Upstream of Box
        gsl_function F;
        F.function = &eubc_neutral_density; 
        
        //! Set Limits of Integration
        D_REAL xmin = -COM_v0/COM_nu;
        D_REAL xmax = -Box_Origin[0]*SI_x0;
        
//         log_file << "Limits of Upstream Region" << endl;
        //! Loop to calculate the velocity
        for(int i=0;i<length_precalc_array;i++)
        {
                //! Calculate radial distance from sun comet lineary
                D_REAL r = max_radius*SI_x0*(D_REAL)i/(length_precalc_array);
//                 D_REAL r = 0;
//                 log_file << "Radius " << r << endl;
                //! Calculate limits of integration
                D_REAL integration_max = -1*sqrt(xmax*xmax+r*r);
                D_REAL integration_min = -1*sqrt(xmin*xmin+r*r);

                
                //! The integral over the mass source:
                //! mass_picked_up = Int( ionisation_frequency * neutral_density)
                D_REAL mass_picked_up = eubc_calc_mass_picked_up(integration_max,integration_min,&F);

                //! Calc Normalised Plasma Parameter
                //! Normalised to Upstream Plasma Values
                D_REAL velocity, massdensity, pressure, magnetic_field;
                //     eubc_calc_massloading_biermann(mass_picked_up,velocity,massdensity,pressure,magnetic_field);   
                eubc_calc_massloading_flammer(mass_picked_up,velocity,massdensity,pressure,magnetic_field);
                
                //! Calc Cometary Ion Number Density
                D_REAL numberdensity_ci;
                D_REAL numberdensity_sw;
                eubc_calc_numberdensities(numberdensity_sw,numberdensity_ci,velocity,massdensity,magnetic_field);
                
                
                //! Write values into arrays
                eubc_precalc_values_velocity[i] = velocity;
                eubc_precalc_values_numberdensity_sw[i] = numberdensity_sw;
                eubc_precalc_values_numberdensity_ci[i] = numberdensity_ci;
                
                //! Report Activation of Extended Upstream Boundary Conditions
                if(i==0)
                {
//                         log_file << "xmin: " << integration_min << endl;
//                         log_file << "xmax: " << integration_max << endl;
                
                        log_file << endl;
                        log_file << "----------------------------------------- " << endl; 
                        log_file << "Set Extended Upstream Boundary Conditions " << endl;
                        log_file << "Mass Picked-Up: "<< mass_picked_up << endl;
                        log_file << "Velocity: " << velocity << endl;
                        log_file << "Mass-Density: " << massdensity << endl;
                        log_file << "Number Density SW: " << numberdensity_sw << endl;
                        log_file << "Number Density CI: " << numberdensity_ci << endl;
                        log_file << "Pressure: " << pressure << endl;
                        log_file << "----------------------------------------- " << endl;
                        log_file << endl;
                }
                
//                 log_file << "Radius " << r << " - Index " << i << " Velocity " << velocity << endl;
                
        }
//         //! Remove 
}

//! Calc Neutral Density Profile
//! Simple version 1/r^2
D_REAL eubc_neutral_density(D_REAL x,void *params) {

    D_REAL neutral_density= COM_nu*COM_Q_Total/(4*pi*COM_v0*x*x);
    //! print request
//     std::cout << "r=" << x << " -- n_n=" << neutral_density << std::endl;
    return neutral_density;
}

//! The integral over the mass source:
//! mass_picked_up = Int( ionisation_frequency * neutral_density)
double CHybrid::eubc_calc_mass_picked_up(double xmax, double xmin, gsl_function *F)
{
        double mass_picked_up;
        double error;
        gsl_integration_workspace *w_integration = gsl_integration_workspace_alloc(100000);
        gsl_integration_qags(F,xmin,xmax,0,1e-7,1000,w_integration,&mass_picked_up,&error);
        gsl_integration_workspace_free(w_integration);
        return mass_picked_up*eubc_mean_neutral_mass;
}

void CHybrid::eubc_calc_massloading_flammer(D_REAL mass_picked_up,D_REAL &velocity,D_REAL &massdensity,D_REAL &pressure, D_REAL &magnetic_field)
{
        //! Use better 
        D_REAL rho_u_inf = m_p*SI_n0*SW_v;
        D_REAL rho_inf = SI_n0*m_p;
        D_REAL u_inf = SW_v;
        D_REAL f = 3;
        D_REAL p_ram_inf = rho_inf*u_inf*u_inf;
        D_REAL p_mag_inf = SI_B0*SI_B0/(2*mu_0);
        D_REAL p_therm_inf = Ion_Betas[0]*p_mag_inf;
        D_REAL P_inf = p_ram_inf + p_mag_inf + p_therm_inf;
        D_REAL E_inf = p_ram_inf/2 + (f+2)*p_therm_inf/2+ 2*p_mag_inf;
        
    
        //! The velocity is solution of a cubic equation
        //! See Notebook Page 120
        
        // Coefficients of Equation
        // lowest order at first
        D_REAL a[4] = { +(f-2.)*p_mag_inf/2.,
                        +E_inf*u_inf, 
                        -(f+2.)*P_inf/2.,
                        +(f+1.)*(mass_picked_up+rho_inf*u_inf)/2.};

        
        //! Solutions 
        gsl_complex z0,z1,z2;
        gsl_poly_complex_solve_cubic(a[2]/a[3], a[1]/a[3], a[0]/a[3], &z0, &z1,&z2);
        //! Solutions sort in ascending order
        //! Subsonic solution is allways smaller than supersonic solution
        //! print solutions
//             std::cout << "Solution Nr. 1: " << GSL_REAL(z0) << " + i * " << GSL_IMAG(z0) << std::endl;
//             std::cout << "Solution Nr. 2: " << GSL_REAL(z1) << " + i * " << GSL_IMAG(z1) << std::endl;
//             std::cout << "Solution Nr. 3: " << GSL_REAL(z2) << " + i * " << GSL_IMAG(z2) << std::endl;
        //! Set velocity and calculate normalised values of velocity, mass density and pressure
        velocity = GSL_REAL(z2)/u_inf;
        magnetic_field = 1/velocity;
        massdensity = (mass_picked_up+rho_inf*u_inf)/(velocity*u_inf*rho_inf);
//         massdensity = 1/velocity; //! Solution with continuity of numberdensities (WRONG)
        pressure = 1-massdensity*velocity*velocity - SI_B0*magnetic_field*SI_B0*magnetic_field/(2*mu_0);
        
}

void CHybrid::eubc_calc_numberdensities(D_REAL &n_sw, D_REAL &n_ci, D_REAL &velocity, D_REAL &rho, D_REAL &magnetic_field)
{
        //! Calc Mass Density Flow of Cometary Ions
        //! See Notebook page 137
        //! Expression for rho_CI * u_x
        
        //! Definition of usefull expression
        D_REAL f = 3;
        D_REAL gamma = (f + 2)/f;
        ...
        D_REAL massdensityflux_ci = (-3.*(f+2.)*P_inf*(1.-1./velocity)+6*E_inf*(1.-1./(velocity*velocity))+3*(f-2.)*p_mag_inf*(1.-1./(velocity*velocity*velocity)))/(3*(f+1)*u_inf);
        
        D_REAL massdensity_ci = massdensityflux_ci/(SW_v*velocity);
        n_ci = (massdensity_ci/eubc_mean_neutral_mass)/SI_n0;
        n_sw = (rho*SI_n0*m_p-massdensity_ci)/(m_p*SI_n0);
}

void CHybrid::delete_extended_upstream_boundary_conditions(void)
{
        delete[] eubc_precalc_values_velocity;
        delete[] eubc_precalc_values_numberdensity_sw;
        delete[] eubc_precalc_values_numberdensity_ci;
}
...
void CHybrid::set_Field_Comps(void)
{
	...
	COMPs_FType[id_in_elastic_en_coll_freq] = 1;
...
void CHybrid::do_consistency_check_init_derived_variables(void)
{
	...
	if( id_extern_PhiC >= NUM_FIELDS)
	{
		log_file << endl << endl << endl;
		log_file << "   id_extern_PhiC"		<< endl
		     ...
		     << id_extern_PhiC +1 << endl
		     << endl;
		exit(1);
	}

	
	INT32 fields_in_use = id_extern_PhiC;

	INT32 field_memory = NUM_SCALAR_FIELDS_EACH_BLK *num_nodes_in_block *sizeof(D_REAL);
	...
	log_file << "sizeof(INT64):      " << sizeof(INT64) << endl;
	...
	if( !vec_len(Magnetic_Moment) && !vec_len(Magnetic_Moment_Star))
	{
		log_file << endl << endl << endl;
	...
	log_file << "Magnetic Moment: " << vec_len(Magnetic_Moment) << endl;
	
	if(vec_len(Magnetic_Moment))
	log_file << "Stand off distance: " << pow((vec_len(Magnetic_Moment)/(vec_len(V_sw)*sqrt(2.))),(1./3.)) << endl;
	
	//! M = sqrt(2.) * vec_len(V_sw) * pow(x_SD,3.).
...
// 		exit(1);
	...
	log_file <<  " SI_t0 = "<<SI_t0<<" s"<<endl<<endl;
	...
	for(INT32  species=0; species < num_Charged_Species; species++)
	{
		...
		log_file << " vth_para (species "<<species<<") = "<< thermal_velocity_para_of[species]<< " = " << thermal_velocity_para_of[species]*SI_v0*1.e-3<<" km/s"<<endl;
		log_file << " vth_perp (species "<<species<<") = "<< thermal_velocity_perp_of[species]<< " = " << thermal_velocity_perp_of[species]*SI_v0*1.e-3<<" km/s"<<endl<<endl;
...
// 	log_file << "   -> Avrg. s/TL (phys. time): " << run_time_phys / TL_processed << "s." << endl;
	...
	//! reduce info via mpi
	D_REAL v_fastest_particle_local[MAX_LEVEL+1];
	D_REAL v_fastest_particle_global[MAX_LEVEL+1];	
        
	for(INT32 level=0;level<=MAX_LEVEL;level++)
	v_fastest_particle_local[level] = sqrt(fastest_particle_v2[level]);

	//! get maximum of all mpi processes
	mpi_myComm.Allreduce(v_fastest_particle_local,v_fastest_particle_global,MAX_LEVEL+1,MPI_D_REAL,MPI_MAX);	

	bool tofast = false;
        for(int level=0;level<MAX_LEVEL+1;level++)
        {	
            log_file << "   -> Level " << level << ": v="<< v_fastest_particle_global[level] << " -> "
                     << delta_of_L[level][0]/(v_fastest_particle_global[level]*dt_particle_of_L[level]) 
                     << " TL to cross cell " << endl;
            if(delta_of_L[level][0]/(v_fastest_particle_global[level]*dt_particle_of_L[level]) < 1.)
                tofast = true;
        }
		...
		if(write_output_if_tofast)
		{	
		
			log_file <<" Writing output to see what is wrong... :-( " << endl;
		
			silo_output_3D();

			//! removing files from last time step
			char buffer[200];
			sprintf(buffer,"rm  %s/silo_3D/*TL%05d*", data_output_path, TL - 1);
