...
void CHybrid::init_Neutral_Profile(void)
{
	...
	INT32 type_to_read = 1;
	
	for(INT32 neutralSpec = 0;neutralSpec<num_Neutral_Species;neutralSpec++)	
	{	
		//! Enceladus
		if(neutral_profile_from_file)	
		read_extern_Profile_uniform_grid(neutralSpec,id_numberdensity_neutralSpecies1+neutralSpec,id_velocity_neutralSpecies1+neutralSpec,type_to_read);
...
void CHybrid::init_IonProduction_Profile(void)
{
	...
	INT32 type_to_read = 2;

	for(INT32 ionProdspecies = 0;ionProdspecies<num_ion_prod_fields;ionProdspecies++)	
	{	
		//! Enceladus
		if(ion_prod_profile_from_file)	
		read_extern_Profile_uniform_grid(ionProdspecies,id_density_ionProdSpecies1+ionProdspecies,id_velocity_ionProdSpecies1+ionProdspecies,type_to_read
		);
		...
		set_ion_production_profile(ionProdspecies);
	}		
...
//! (in this order)
//!--------------------------------------------------------
void CHybrid::read_extern_Profile_uniform_grid(INT32 species, INT32 id_densityfield, INT32 id_velocityfield, INT32 TYPE)
{
	...
	if(TYPE==1)
	{
		log_file << " Reading extern Neutral Profile from file:" << endl <<"  "
		<< extern_NeutralField_name[species] << endl
	...
	if(TYPE==2)
	{
		log_file << " Reading extern Ion Production Profile from file:" << endl <<"  "
		<< extern_IonProdField_name[species] << endl
		<< " and storing it to field '"
		<< Field_Name[id_densityfield] << " ' ...  " << endl;
	}
#endif
	...
	if(TYPE==3)
	{
		log_file << " Reading extern Dust Profile from file:" << endl <<"  "
		<< extern_DustField_name[species] << endl
		<< " and storing it to field '"
		<< Field_Name[id_densityfield] << " ' ...  " << endl;
	}
#endif
	...
	FILE_REAL *extern_rho, *extern_UiX, *extern_UiY, *extern_UiZ;


	char full_filename[200];
        
	if(TYPE == 1)
	sprintf(full_filename,"%s",extern_NeutralField_name[species]);

#if defined(use_ion_production_as_field) 
	if(TYPE == 2)
        sprintf(full_filename,"%s",extern_IonProdField_name[species]);
#endif
	
#if defined(use_dust_species_as_field)
	if(TYPE == 3)
	sprintf(full_filename,"%s",extern_DustField_name[species]);
#endif
...
// 	extern_rho_file.seekg(3*num_extern_box_nodes*sizeof(FILE_REAL),ios::cur);
	...
	{	
		...
		{
			//! different normalizations possible (dust vs neutrals)
			extern_rho[a_b_c] *= norm_externNeutralDensity[species];
			extern_UiX[a_b_c] *= norm_externNeutralVelocity[species];
			extern_UiY[a_b_c] *= norm_externNeutralVelocity[species];
			extern_UiZ[a_b_c] *= norm_externNeutralVelocity[species];
		}
	}
	...
	{	
		...
		{
			//! different normalizations possible (dust vs neutrals)
			extern_rho[a_b_c] *= norm_externDustDensity[species];
			extern_UiX[a_b_c] *= norm_externDustVelocity[species];
			extern_UiY[a_b_c] *= norm_externDustVelocity[species];
			extern_UiZ[a_b_c] *= norm_externDustVelocity[species];
		}
	}
#endif
