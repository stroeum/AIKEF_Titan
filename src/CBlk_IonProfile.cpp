...
{
	//! comet
	if(vec_len2(r_normed) > COM_a*COM_a)
	return true;
	else
	return false;
}
...
// 						getIonProdVelocityAtPoint(x_part,cell_indices,neutralSpec,neutralSpec,v_part);					
						...
						//!MF: Thermal velocity of neutrals is negligible to plasma vel anyway
  ...
  INT32 end[3]={BlkNds_X-1 -!a*((BlkNds_X-2)/2),
       ...
       for(INT32 i = start[0]; i < end[0]; i++)
		...
		for(INT32 neutralSpec=0; neutralSpec<num_Neutral_Species; neutralSpec++)
		 ...
		 if(PhotoionisationRate[species][neutralSpec]>0 || ElectronionisationRate[species][neutralSpec]>0)	 
		 {
			//! Füge nur zufällig alle insert_every_x_TL Zeitschritte ein MP ein
			D_REAL zufall =  1.*random()/RAND_MAX; 
			if(zufall<1./(1.*insert_every_x_TL)) 
			{
				...
				{	
					...
					weight_part = insert_every_x_TL*(IonProdRate_at_point
									+neutral_density_at_point*ElecIoRate);
					...
					if(weight_part > 0.)
					{
						...
						new_ion.weight = weight_part *CellVol_of_L[RLevel]*dt;
